...
public class TestListener implements ITestListener, ISuiteListener, IExecutionListener {
	...
	public void onTestFailure(ITestResult result) {
		...
		DriverSession.getExtent().log(Status.FAIL,
				MarkupHelper.createLabel(
						"TC " + getExecutingMethodName(result) + " Failed Reason \n" + getThrowableMessage(result),
						ExtentColor.RED));
		String screenshotPath = GlobalParam.CURRENT_PROJECT_PATH + GlobalParam.slash + "Screenshots" + GlobalParam.slash
				+ result.getName() + ".png";
		
		if (GlobalParam.screenshotOption.equals("api")){
	...
	public void onTestSkipped(ITestResult result) {
		DriverSession.getExtent().log(Status.SKIP,
				MarkupHelper.createLabel("TC " + getExecutingMethodName(result) + "Skipped!", ExtentColor.YELLOW));
